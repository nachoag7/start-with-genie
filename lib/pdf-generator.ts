...
function addAppleFooter(doc: jsPDF) {
  ...
  doc.text('Generated by Start With Genie · Your silent assistant for setup', pageWidth / 2, y, { align: 'center' })
  doc.setTextColor(30)
}
...
function addGenieWatermark(doc: jsPDF) {
  // Add subtle genie icon watermark (placeholder for now)
  const pageCount = doc.getNumberOfPages()
  for (let i = 1; i <= pageCount; i++) {
    ...
    // Add subtle watermark text instead of image for now
    doc.setFontSize(8)
    doc.setTextColor(200)
    doc.text('SWG', pageWidth - 20, pageHeight - 15)
    doc.setTextColor(30)
  }
}
...
export async function generateLLCFilingInstructions(data: DocumentData & { managerType?: string }): Promise<string> {
  ...
  y += 8
  doc.text('Start With Genie - Your silent assistant for setup', 40, y)
  y += 8
  ...
  addAppleFooter(doc)
  addGenieWatermark(doc)
  
  const pdfBlob = doc.output('blob')
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('User not authenticated')
  
  const fileName = `LLC-Filing-Instructions-${Date.now()}.pdf`
...
export async function generateEINGuide(data: DocumentData & { einStatus?: string, llcType?: string }): Promise<string> {
  ...
  // Section 1
  y = addAppleHeading(doc, '1. What Is an EIN?', 40, y)
  y = addAppleText(doc, `An EIN (Employer Identification Number) is a unique ID issued by the IRS.\nThink of it as your business's Social Security Number - it's required to:\n\n- Open a business bank account\n- File taxes\n- Hire employees\n- Apply for business credit\n\nEven if you're the only owner, most banks and services will ask for your EIN.`, 40, y, { spacing: 8 })
  y += 16

  // Section 2
  y = addAppleHeading(doc, '2. Do You Need One?', 40, y)
  y = addAppleText(doc, `Based on your answers: ${einStatus}\n\nMost LLCs do need an EIN. Even if you're a single-member LLC with no employees, you'll likely need it for banking, taxes, or applying for licenses.\n\nGood news: getting an EIN is completely free and only takes a few minutes.`, 40, y, { spacing: 8 })
  y += 16

  // Section 3
  y = addAppleHeading(doc, '3. How to Apply Online (Recommended)', 40, y)
  y = addAppleText(doc, `The IRS provides a free online application for EINs.\n\nWhere to apply:\nhttps://irs.gov/ein\n\nWhen:\nMonday through Friday, 7:00 AM - 10:00 PM EST\n\nWhat you'll need:\n\nYour name: ${data.fullName}\n\nYour business name: ${data.businessName}\n\nBusiness address\n\nBusiness structure: ${llcType}\n\nWhether you are the owner\n\nWhat to expect:\nThe application takes about 10 minutes. You'll receive your EIN immediately after submission.`, 40, y, { spacing: 8 })
  y += 16

  // Section 4
  y = addAppleHeading(doc, '4. After You Apply', 40, y)
  y = addAppleText(doc, `Once you get your EIN:\n\n- Save the confirmation letter (CP 575) - this is your proof of EIN\n- You'll need it to open a business bank account, file taxes, and set up payroll\n- Come back to your Genie Dashboard to download your Operating Agreement`, 40, y, { spacing: 8 })
  y += 16

  // Section 5
  y = addAppleHeading(doc, '5. Alternate Filing (Mail or Fax)', 40, y)
  y = addAppleText(doc, `If you don't have a Social Security Number or can't use the online form:\n\n- Download Form SS-4\n- Fill it out and send it by mail or fax to the IRS\n\nThis method takes longer, but works for non-U.S. residents and others with special cases.`, 40, y, { spacing: 8 })
  ...
  const fileName = `EIN-Guide-${Date.now()}.pdf`
  return await uploadPDFToSupabase(pdfBlob, fileName, user.id)
}
...
export async function generateOperatingAgreement(data: DocumentData & {
  ...
  const fileName = `Operating-Agreement-${Date.now()}.pdf`
  return await uploadPDFToSupabase(pdfBlob, fileName, user.id)
}